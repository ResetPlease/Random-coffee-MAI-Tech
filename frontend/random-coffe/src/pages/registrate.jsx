...
import { getAuthParams, getURLtoEmailVetify, setTokens } from '../utils/auth'
...
const RegisterPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.first_name.length === 0) {
            handleErrorsChange('Необходимо заполнить поле', 'first_name');
            return;
        }

        if (formData.last_name.length === 0) {
            handleErrorsChange('Необходимо заполнить поле', 'last_name');
            return;
        }
        ...
        if (response.status === 400) {
            ...
            if (type === 'email_occupied'){
                handleErrorsChange('Пользователь уже зарегистрирован', 'email');
                return;
            }
  ...
  return (
    <div style={defaultStyles.container}>
      <div style={defaultStyles.card}>
        <h2 style={defaultStyles.title}>Регистрация</h2>
        ...
        <form onSubmit={handleSubmit} style={defaultStyles.form}>
          <input
            type="text"
            placeholder="Имя"
            style={{...defaultStyles.input, borderColor: errors.first_name === false ? '#1e90ff' : 'red'}}
          ...
          <input
            type="text"
            placeholder="Фамилия"
            style={{...defaultStyles.input, borderColor: errors.last_name === false ? '#1e90ff' : 'red'}}
